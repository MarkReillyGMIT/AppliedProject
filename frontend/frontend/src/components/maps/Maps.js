...
export default class Maps extends React.Component {
  ...
  async getWeather() {
    try {
      const api_call = await fetch(`//api.openweathermap.org/data/2.5/weather?lat=${this.state.markers[0].lat}&lon=${this.state.markers[0].lng}&appid=4c877f2673f01a25cab9395e71356dad`)
  ...
  async getForecast() {
    const forecast_call = await fetch(`//api.openweathermap.org/data/2.5/forecast?lat=${this.state.markers[0].lat}&lon=${this.state.markers[0].lng}&appid=4c877f2673f01a25cab9395e71356dad`)
  ...
  componentDidUpdate() {
    if (this.state.markers[0] == undefined) {
      ...
      console.log("Map" + map.invalidateSize());
